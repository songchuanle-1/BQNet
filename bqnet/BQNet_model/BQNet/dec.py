...
import ipdn.torch.nn_new.functional as F
from ipdn.torch.nn_new import MultiheadAttention
...
class DEC(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layer):
            ...
            # self.scg.append(MHA(d_model, d_k, d_v, nhead, dropout)) ####
        ...
        # self.scg_head = MHA(d_model, d_k, d_v, nhead, dropout)
    ...
    def forward_iter_pred(self, x, fps_seed_sp, batch_offsets, lang_feats=None, lang_masks=None, sp_pos=None, feats_2d=None):
        ...
        for i in range(self.num_layer):
            # scg_weights, scg_mask = self.get_scg_weight(query_pos, k=(self.num_layer - i + 2) * 4)
            scg_weights, _ = self.get_scg_weight(query_pos, k=None)
            ...
            query_qia, _, _ = self.qia_layers[i](inst_feats, query, batch_mask, attn_masks)
            query_qla, _, _ = self.qla_layers[i](lang_query, query_qia, lang_masks)
            query_self = self.qqa_layers[i](query_qla)
            query = self.norm_qqa(query + query_self)

            query = self.scg[i](query, att_weights = scg_weights)

            query = self.qqa_ffn_layers[i](query)
            query_norm = self.query_norm(query)
            query_pos_offset = self.offset(query_norm) #[num_queries, bsz, 3] 
            query_pos += query_pos_offset
    ...
    def get_scg_weight(self, pos_q, pos_k=None, k_mask=None, k=None):
        ...
        dis_min = dis.min(dim=-1, keepdim=True)[0]
        dis_max = dis.max(dim=-1, keepdim=True)[0]
        dis_normalized = (dis - dis_min) / (dis_max - dis_min + 1e-6)
        
        # 步骤 2: 应用高斯核（距离越小→权重越大）
        gamma = self.gamma  # 归一化后，gamma 可固定为较大值以增强区分度
        scg_weight = torch.exp(-gamma * dis_normalized)
