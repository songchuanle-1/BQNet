...
class Criterion(nn.Module):
    ...
    def triple_loss(self, inputs, indices):
        
        targets = torch.zeros(inputs.size(0), inputs.size(1))
        for idx, (idx_q, idx_gt) in enumerate(indices):
            targets[idx, idx_q] = 1
        bs, n, c = inputs.shape

        # 计算欧氏距离矩阵
        distance = torch.cdist(inputs, inputs, p=2)
        mask = targets.unsqueeze(2) == targets.unsqueeze(1)  # 同类掩膜 (bs*n, bs*n)

        self_mask = torch.eye(n, device=inputs.device, dtype=torch.bool).unsqueeze(0).repeat(bs,1,1).to(mask.device) # 自身掩膜

        # 1. 最难正样本（排除自身）
        pos_mask = mask & ~self_mask  # 同类且非自身   
        pos_distance = distance.clone()
        pos_distance[~pos_mask] = -torch.inf  # 非正样本对设为极小值
        distance_ap = pos_distance.max(dim=-1).values  # 

        # 2. 最难负样本
        neg_mask = ~mask  # 异类样本对
        neg_distance = distance.clone()
        neg_distance[~neg_mask] = torch.inf  # 非负样本对设为极大值
        distance_an = neg_distance.min(dim=-1).values  # (bs,n)


        # 3. 过滤无效样本（无正样本或负样本）
        valid = (distance_ap != -torch.inf) & (distance_an != torch.inf)
        if not valid.any():
            return torch.tensor(0.0, device=inputs.device, requires_grad=True)
        
        distance_ap = distance_ap[valid]
        distance_an = distance_an[valid]
    ...
    def get_layer_loss(self, layer, aux_outputs, pad_masks, target, indices=None, lang_masks=None):
        ...
        triple_loss = self.triple_loss(proj_queries, indices)
        ...
        loss_out['triple_loss'] = triple_loss
        ...
        loss = (
            ...
            self.loss_weight[7] * triple_loss)
